...
SSH_KEY_LOCATION="~/.ssh/id_ed25519_leonhard"
...
WAITING_TIME_INTERVAL=15

# Name of the folder we are working in, eg where the jupyter notebook is located with respect to the home directory ~
PROJECT_NAME="cil_2021"
#############################
...
function print_usage {
        echo -e "Usage:\t start_jupyter_nb.sh CLUSTER NETHZ_USERNAME NUM_CORES RUN_TIME MEM_PER_CORE NUM_GPUS\n"
        echo -e "Arguments:\n"
	...
	echo -e "NUM_GPUS\t\tNumber of GPUs to be used on the cluster (students normaly have at most 1)"
        echo -e "Example:\n"
        echo -e "./start_jupyter_nb.sh Euler sfux 4 01:20 2048 0\n"
}
...
if [ "$CLUSTERNAME" == "Euler" ]; then
    ...
    PCOMMAND="r/3.5.1 python_cpu/3.6.4 eth_proxy python_gpu/3.7.4"
else
...
ssh $SSH_KEY_OPTION $USERNAME@$CHOSTNAME <<ENDBSUB
module load $PCOMMAND
bsub -n $NUM_CORES -W $RUN_TIME -R "rusage[mem=$MEM_PER_CORE, ngpus_excl_p=$NUM_GPUS]"
export XDG_RUNTIME_DIR=
IP_REMOTE="\$(hostname -i)"
echo "Remote IP:\$IP_REMOTE" >> /cluster/home/$USERNAME/jnbip
jupyter notebook --no-browser --ip "\$IP_REMOTE" --notebook-dir "~/$PROJECT_NAME" &> /cluster/home/$USERNAME/jnbinfo 
ENDBSUB
